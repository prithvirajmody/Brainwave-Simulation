...
expName = 'prototype_exp'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\prith\\OneDrive\\Desktop\\PRITHVIRAJ MODY\\UC Davis\\Clubs\\Neurotech\\brainwave simulation\\prototype_exp_lastrun.py',
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() < 3.0:
            ...
            if fixation_cross.status == STARTED:
                ...
                if tThisFlipGlobal > fixation_cross.tStartRefresh + 3.0-frameTolerance:
        ...
        Stimulus = data.Routine(
            name='Stimulus',
            components=[red_circle],
        )
        Stimulus.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from time_logger
        import time
        thisExp.addData('stimulus_onset', time.time())
        ...
        while continueRoutine and routineTimer.getTime() < 5.0:
            ...
            # if red_circle is starting this frame...
            if red_circle.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                red_circle.frameNStart = frameN  # exact frame index
                red_circle.tStart = t  # local t and not account for scr refresh
                red_circle.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(red_circle, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'red_circle.started')
                # update status
                red_circle.status = STARTED
                red_circle.setAutoDraw(True)
            
            # if red_circle is active this frame...
            if red_circle.status == STARTED:
                # update params
                pass
            
            # if red_circle is stopping this frame...
            if red_circle.status == STARTED:
                # is it time to stop? (based on local clock)
                if tThisFlip > 5.0-frameTolerance:
                    # keep track of stop time/frame for later
                    red_circle.tStop = t  # not accounting for scr refresh
                    red_circle.tStopRefresh = tThisFlipGlobal  # on global time
                    red_circle.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'red_circle.stopped')
                    # update status
                    red_circle.status = FINISHED
                    red_circle.setAutoDraw(False)
        ...
        while continueRoutine and routineTimer.getTime() < 3.0:
            ...
            if rest_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rest_text.tStartRefresh + 3.0-frameTolerance:
                    # keep track of stop time/frame for later
